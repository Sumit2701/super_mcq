...
export default function Home({username}) {
  ...
  const handleQuizSelection = (quiz) => {
    const selectedQuiz = quizOptions.find(option => option.name === quiz);
    if (selectedQuiz) {
      
      router.push({
        pathname: '/questions',
        query: { quiz: selectedQuiz.quizname, numQuestions: selectedQuiz.numQuestions },
      });
    }
  };
            ...
            {quizOptions.map(quiz => (
              <Col key={quiz.id} xs={24} sm={12} md={8} lg={6}>
                <Card
                  ...
                  style={{ width: '100%', background:'linear-gradient(to Right,#deecfb,	#7ab3ef)'}}
