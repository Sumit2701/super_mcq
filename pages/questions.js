...
const Questions = () => {
  ...
  const fetchQuestions = async (quiz, numQuestions) => {
    try {
      const response = await fetch(
        `/api/questions?quiz=${quiz}&numQuestions=${numQuestions}`
      );
  ...
  const getOptionResultText = (questionIndex, optionIndex) => {
    if (
      ...
      const selectedOption =
        questions[questionIndex].options[selectedOptions[questionIndex]];
  ...
  const getCorrectOptionStyle = (question, option, questionIndex) => {
    if (submitted[questionIndex]) {
      return option === question.correctAnswer ? styles.optionCorrect : "";
    }
  };
        ...
        <Layout style={{ backgroundColor: "#F9F9F9" }}>
          ...
          <Content className={styles.content}>
            ...
            {questions.map((question, questionIndex) => (
              <Card
                key={questionIndex}
                className={`${styles.card} ${getQuestionCardStyle(
                  questionIndex
                )}`}
                title={<div className={styles.question}>{question.question}</div>}
              >
                {" "}
